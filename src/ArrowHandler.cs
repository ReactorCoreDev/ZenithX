...
public static class ArrowHandler
{
		...
		foreach (NormalPlayerTask[] array2 in array)
		{
			foreach (NormalPlayerTask val in array2)
			{
				if ((Object)(object)val.Arrow != (Object)null)
				{
					...
					{
						ZenithX.Log($"Cached Arrow gameObject from ShipStatus for task {((PlayerTask)val).TaskType}");
					}
					return;
				}
				if (CheatToggles.debugMode)
				{
					ZenithX.Log($"No Arrow gameObject found on task {((PlayerTask)val).TaskType}");
				}
			}
		}
	}
	...
	public static ArrowBehaviour CreateArrowForTask(NormalPlayerTask task)
	{
		...
		if (CheatToggles.debugMode)
		{
			ZenithX.Log($"Creating task arrow by cloning cached template for task {((PlayerTask)task).TaskType}");
		}
		GameObject obj = Object.Instantiate<GameObject>(_cachedArrowTemplate, ((Component)task).transform, false);
		((Object)obj).name = "ZenithXArrow";
	...
	public static void SetArrowTargetForSpecialTasks(NormalPlayerTask task)
	{
		...
		else if ((int)taskType != 55)
		{
			if ((int)taskType != 66)
			{
				...
				{
					Console targetConsole2 = ((IEnumerable<Console>)val2.ToArray()).FirstOrDefault((Func<Console, bool>)((Console c) => c.ConsoleId == ((Il2CppArrayBase<byte>)(object)task.Data)[0]));
			...
			else
			{
				...
				{
					Console targetConsole3 = ((IEnumerable<Console>)val3.ToArray()).FirstOrDefault((Func<Console, bool>)((Console c) => c.ConsoleId == ((Il2CppArrayBase<byte>)(object)task.Data)[0]));
